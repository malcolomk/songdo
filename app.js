...
function onOrderArtNoInput(artNoValue) {
  ...
  if (artNameMatch) {
    ...
    if (currentStock > 0) {
      showToast(`창고에 재고가 ${currentStock}개 있습니다! 오더 대신 창고 재고를 챙겨주세요.`, "success");
    }
  } else {
...
async function handleRegisterSubmit(e) {
  ...
  try {
    ...
    showToast(`${type} ${qty}개가 기록시트에 추가되었습니다!`, "success", insertedId);
...
window.undoLastAction = async function(id, toastElem) {
  ...
  if (tempToast.parentNode) tempToast.parentNode.removeChild(tempToast);
  showToast("방금 한 작업이 성공적으로 취소되었습니다.", "success");
}
