...
function handleLoginSubmit(e) {
  ...
  showToast(`${matchedId}님 (${roleTitle}), 로그인되었습니다!`, "success");
...
async function saveHistoryLogs(log) {
  if (supabaseClient) {
    try {
      ...
      if (dbLog.artName) { dbLog.artname = dbLog.artName; delete dbLog.artName; }
      await supabaseClient
        .from("inventory_logs")
        .insert([dbLog]);
    } catch (err) {
...
async function saveOrderLogs(order) {
  if (supabaseClient) {
    try {
      const dbOrder = { ...order };
      if (dbOrder.artNo) { dbOrder.artno = dbOrder.artNo; delete dbOrder.artNo; }
      if (dbOrder.artName) { dbOrder.artname = dbOrder.artName; delete dbOrder.artName; }
      await supabaseClient
        .from("order_requests")
        .insert([dbOrder]);
    } catch (err) {
