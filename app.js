...
function quickLogin(userId) {
  ...
  if (loginIdElem) loginIdElem.value = userId;
  if (loginPwElem) loginPwElem.value = INITIAL_PASSWORD;

  localStorage.setItem("warehouse_current_user", userId);
  const roleTitle = ADMIN_USERS.includes(userId) ? "👑 관리자" : "일반 사용자";
  showToast(`${userId}님 (${roleTitle}), 로그인되었습니다!`, "success");
  checkLoginSession();
}
...
function checkLoginSession() {
  ...
  if (sessionUser && ALLOWED_USER_IDS.includes(sessionUser)) {
    ...
    try { renderMasterCatalog(); } catch (e) { console.error(e); }
...
function handleLoginSubmit(e) {
  ...
  const inputId = loginIdElem.value.trim().toLowerCase();
  ...
  if (inputPw === storedPw || inputPw === INITIAL_PASSWORD || inputPw === "522") {
