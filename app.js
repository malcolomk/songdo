...
function loadDataFromStorage() {
  ...
  // Clear sample logs for actual production usage if requested
  const prodResetFlag = localStorage.getItem("warehouse_prod_reset_v3");
  if (!prodResetFlag) {
    historyLogs = [];
    orderLogs = [];
    saveHistoryLogs();
    saveOrderLogs();
    localStorage.setItem("warehouse_prod_reset_v3", "true");
  } else {
    try {
    ...
    try {
      ...
      orderLogs = savedOrders ? JSON.parse(savedOrders) : [];
    } catch (err) {
      orderLogs = [];
    }
  }
