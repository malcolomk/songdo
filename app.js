  ...
  try {
    ...
    if (data) {
      data.forEach(inserted => {
        historyLogs.unshift(inserted);
        if (!masterCatalogMap.has(inserted.artNo)) {
          masterCatalog.push({ artNo: inserted.artNo, artName: inserted.artName || "신규 품목" });
          saveMasterCatalog();
        }
      });
      populateArticleFilterDropdown();
      ...
      html += `
        <div class="history-item" style="border-left-color: #f59e0b; background-color: #fffbeb;">
          ...
          <div class="hist-right" style="align-items:flex-end;">
            ...
            <button type="button" class="btn-submit" style="background-color: #059669; font-size: 11px; padding: 6px 12px; margin-top: 6px; width: auto;" onclick="completePickItem(${index})">
...
async function completePickItem(index) {
  ...
  try {
    ...
    if (window.supabaseClient && pickItem.id) {
